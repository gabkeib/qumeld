...
def PH_Mahattan(parr):
    ...
    t0 = ctime()
    a1 = depth_oriented_scheduling(parr, length=length, maxiter=30)
    qc = synthesis_SC.qiskit_synthesis(
        a1, initial_layout=list(range(lnq)), arch="manhattan"
    )
    ...
    a2 = gate_count_oriented_scheduling(parr)
    qc = synthesis_SC.qiskit_synthesis(
        a2, initial_layout=list(range(lnq)), arch="manhattan"
    )
    ...
    print("DO vs GCO", end=" ")
    print(
        f"CNOT: {(c0 / c1 - 1):.2%}, Single: {(s0 / s1 - 1):.2%}, Total: {((c0 + s0) / (c1 + s1) - 1):.2%}, Depth: {(d0 / d1 - 1):.2%}"
    )
    print("Block-Wise Compilation vs Qiskit")
    print(
        f"CNOT: {(c0 / c2 - 1):.2%}, Single: {(s0 / s2 - 1):.2%}, Total: {((c0 + s0) / (c2 + s2) - 1):.2%}, Depth: {(d0 / d2 - 1):.2%}"
    )
