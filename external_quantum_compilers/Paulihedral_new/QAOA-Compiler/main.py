...
parser.add_argument(
    "-device_json",
    help="Target Device configuration File",
    type=str,
    default=None,
    action="store",
    dest="QCD",
...
parser.add_argument(
    "-circuit_json",
    help="Problem QAOA ZZ Interaction File",
    type=str,
    default=None,
    action="store",
    dest="CKT",
...
parser.add_argument(
    "-config_json",
    help="QAOA/Compiler configuration File",
    type=str,
    default=None,
    action="store",
    dest="CON",
...
parser.add_argument(
    "-target_IterC",
    help="Chosen minimization objective"
...
if __name__ == "__main__":
    if (
        os.path.isfile(args.QCD)
        and os.path.isfile(args.CKT)
        and os.path.isfile(args.CON)
    ):
        comp_obj = cqq.CompileQAOAQiskit(
            circuit_json=args.CKT,
            qc_json=args.QCD,
            config_json=args.CON,
            out_circuit_file_name=args.OUT,
        )
        if args.POL == "IP":
            comp_obj.run_ip()
        elif args.POL == "IterC":
