...
class CompileQAOAQiskit:
    def __init__(
        ...
        qc_json=None,
        config_json=None,
        out_circuit_file_name="QAOA.qasm",
    ):
        """
    ...
    def _naive_compilation(self):
        ...
        trans_ckt = transpile(
            ...
            seed_transpiler=self.Trans_Seed,
    ...
    def _calc_qq_distances(self):
        ...
        self.noise_aware_qq_distances = nx.floyd_warshall(
            self.weighted_undirected_coupling_graph
        )
    ...
    def _instruction_parallelization(self, sorted_edges=None, single_layer=False):
        ...
        for qubit in logical_qubits:
            layer_occupancy[current_layer].insert(
                len(layer_occupancy[current_layer]), [qubit, "FREE"]
            )
            ...
            for edge in remaining_edges:
                ...
                for occupancy_info_list in layer_occupancy[current_layer]:
                    ...
                    if free_among_the_two == 2:
                        ...
                        self.layer_zz_assignments[current_layer].insert(
                            len(layer_occupancy[current_layer]), edge
                        )
                        allocated_op_count_in_this_layer = (
                            allocated_op_count_in_this_layer + 1
                        )
                        break
    ...
    def _iterative_compilation(self):
        ...
        while True:
            ...
            layer_order[layer_1], layer_order[layer_2] = (
                layer_order[layer_2],
                layer_order[layer_1],
            )
            ...
            if cir._data:
                ...
                if len(qub) == 1:
                    ...
                    if "({},{})".format(qub[0], qub[1]) in self.qc_data[gate].keys():
                        ESP = ESP * float(
                            self.qc_data[gate]["({},{})".format(qub[0], qub[1])]
                        )
                    elif "({},{})".format(qub[1], qub[0]) in self.qc_data[gate].keys():
                        ESP = ESP * float(
                            self.qc_data[gate]["({},{})".format(qub[1], qub[0])]
                        )
                    else:
                        print(
                            "Please check the device configuration"
                            + "file for the following qubit-pair data: {}, {}".format(
                                qub[0], qub[1]
                            )
                        )
            else:
    ...
    def _qasm_note(self, ckt=None, pol=None):
        ...
        """
        assert self._approximate_equivalence(
            ckt
        ...
        if ckt:
            ...
            self._estimate_sp()
            print(
                "(naive) Depth: {}, gate-count(2Q): {}, ESP: {}".format(
                    self.naive_ckt.depth(),
                    self.naive_ckt.count_ops()[self.native_2q[0]],
                    math.exp(-self.cost),
                )
            )
            ...
            self._estimate_sp()
            print(
                "({}) Depth: {}, gate-count(2Q): {}, ESP: {}".format(
                    pol,
                    ckt.depth(),
                    ckt.count_ops()[self.native_2q[0]],
                    math.exp(-self.cost),
                )
            )

            print(
                "The circuit is written with beta/gamma parameters"
                + "at different P-lavels (https://arxiv.org/pdf/1411.4028.pdf)"
            )
            print("bX --> beta parameter at p=X")
            print(
                "gX_Y_Z --> gamma parameter at p=X between *logical* "
    ...
    def run_ip(self):
        ...
        print(
            "Instruction Parallelization-only Compilation (IP) completed!"
    ...
    def run_iter_c(self, target="D"):
        ...
        print(
            "Iterative Compilation (IterC) completed!"
    ...
    def run_incr_c(self, variation_aware=False):
        ...
        if variation_aware:
            ...
            print(
                "Incremental Compilation (IC) completed!"
