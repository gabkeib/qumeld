...
def simple_seq_synthesis1(ps_layers, time_parameter=1):
    ...
    while i < nl:
        ...
        syn_pauli_string(
            qc, nq, ps_layers[i][0][0].ps, cnotset, root, ps_layers[i][0][0].real
        )
    ...
    for i in edge:
        if i[0] == i[1]:
            ...
            syn_pauli_string(
                qc,
                nq,
                ps_layers[i[0]][0][0].ps,
                cnotset,
                root,
                ps_layers[i[0]][0][0].real,
            )
            ...
            syn_pauli_string(
                qc,
                nq,
                ps_layers[i[1]][0][0].ps,
                cnotset,
                root,
                ps_layers[i[1]][0][0].real,
            )
...
def qiskit_synthesis(ps_layers, time_parameter=1):
    ...
    for i in ps_layers:
        for j in i:
            for k in j:
                qc.append(
                    evolution_instruction([[1, Pauli.from_label(k.ps[::-1])]], 1, 1),
                    qc.qubits,
                )
    return qc
