...
def create_feature_map(
    num_qubits: int, features: NDArray[np.float64]
) -> QuantumCircuit:
...
class QMLCostFunction:
    ...
    def __call__(self, params):
        ...
        if self.iteration % 1 == 0:
            print(
                f"Iteration {self.iteration}: Cost = {avg_cost:.6f}, Params[0:3] = {params[:3]}"
            )
...
def run_qml_example():
    ...
    cost_func = QMLCostFunction(
        X_train, y_train, num_qubits, num_layers, observable, backend, qubit_mapper
    )
