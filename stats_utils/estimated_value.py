...
def pad_hamiltonian_to_size(hamiltonian: SparsePauliOp, target_num_qubits: int) -> SparsePauliOp:
    ...
    if current_qubits > target_num_qubits:
        raise ValueError(f"Cannot pad: Hamiltonian has more qubits ({current_qubits}) than target ({target_num_qubits})")
