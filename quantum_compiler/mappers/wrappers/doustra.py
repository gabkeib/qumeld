...
class Doustra(QubitMapper):
    ...
    def doustra_optimise_circuit(
        self, circuit: QuantumCircuit, backend: BackendV2, quantum_algorithm: str
        ...
        out_path = (
            project_root / "dumps" / f"{quantum_algorithm_normalized}_output.qasm"
        )
        layout_path = (
            project_root
            / "configs"
        ...
        if circuit.parameters:
            ...
            parameter_values = {
                param: np.random.uniform(0, 2 * np.pi) for param in circuit.parameters
            }
            # Or
        ...
        if not Path(layout_path).exists():
            ...
            self.convert_to_layout(
                backend_name_normalized, edge_list_ll, backend.num_qubits
            )
        ...
        command = [qsyn_path, script_path, in_path, layout_path, out_path]
    ...
    def convert_to_layout(
        self, name, quantum_computer: List[List[int]], num_qubits: int
    ) -> List[int]:
        project_root = get_project_root()
        layout_path = (
            project_root / "configs" / "doustra" / "layouts" / f"{name}.layout"
        )
