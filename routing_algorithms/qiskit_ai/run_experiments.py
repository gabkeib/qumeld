...
from experiments.run_qiskit import CustomBackend, convert_pauli_strings
...
def qiskit_ai_optimise_circuit(circuit: QuantumCircuit, backend: CustomBackend) -> QuantumCircuit:
    # print("coupling map in qiskit ai:", backend.coupling_map)
    # ai_transpiler_pass_manager: PassManager = generate_ai_pass_manager(
    #     backend=backend,
...
def run_qiskit_ai_hamiltonian(quantum_computer_backend, num_qubits, pauli_strings, algorithm_name):
    time_start = time()
    ...
    backend = reconstruct_backend(quantum_computer_backend)
    # print("NOT HERE")
    # print("Converted strings:", converted_strings)

    evo_gate = PauliEvolutionGate(hamiltonian, time=1.0)
    circuit = QuantumCircuit(hamiltonian.num_qubits)
    circuit.append(evo_gate, circuit.qubits)

    # print("Run number of qubits:", backend.num_qubits)
    # print("Qubits in circuit:", circuit.num_qubits)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if function_name == 'run_qiskit_ai_hamiltonian':
            result = run_qiskit_ai_hamiltonian(*args, **kwargs)
        elif function_name == 'run_qiskit_ai_circuit':
