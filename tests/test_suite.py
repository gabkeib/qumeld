...
class CorrectnessTestSuite:
    ...
    def _validate_with_fallback(self, original: QuantumCircuit, 
                                mapped: QuantumCircuit,
                                preferred_method: str) -> tuple[bool, str, str]:
        """
        ...
        if preferred_method == 'blind':
            is_valid, fidelity, inferred_layout = self.blind_validator.validate(original, mapped)
            return is_valid, f"Fidelity: {fidelity}, First qubit mapped: {inferred_layout.get(0, 'unknown')}", 'blind'
            ...
            return correct, f"Statevector failed, used measurement: {msg}", 'measurement'
    ...
    def test_algorithm(self,
        ...
        if algorithm.supports_circuit_mapping():
            for i, circuit in enumerate(circuits, 1):
                ...
                # Apply algorithm
                mapped = algorithm.map_circuit(circuit, backend=self.backend, circuit_name="test_mapping")
                if mapped is None:
                    if verbose:
                    ...
                    continue
                # Test
                result = self.test_circuit(circuit, mapped.optimised_circuit, method, verbose=False)
        ...
        if algorithm.supports_raw_pauli_string_mapping():
            for i, pauli_strings in enumerate(pauli_strings_set, 1):
                circuit = convert_pauli_strings_to_circuit(pauli_strings, num_qubits=len(pauli_strings[0]))
                mapped = algorithm.map_pauli_strings(pauli_strings, backend=self.backend, circuit_name="test_mapping")
    ...
    def _print_results(self, results: dict):
        ...
        print(f"Gates: {results['original_gate_count']} → {results['mapped_gate_count']}")
        print(f"Method: {results['test_method']}")
        print(f"\nResults:")
        print(f"  Topology compliant: {'✓' if results['topology_compliant'] else '✗'}")
        print(f"  Functionally correct: {'✓' if results['functionally_correct'] else '✗'}")
        print(f"\nDetails:")
