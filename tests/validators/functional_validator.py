...
class FunctionalValidator:
    ...
    def validate_statevector(original: QuantumCircuit, 
        ...
        try:
            ...
            if num_qubits_mapped > num_qubits_orig:
                ...
                return False, f"Mapped circuit has fewer qubits ({num_qubits_mapped}) than original ({num_qubits_orig})"
    ...
    def validate_unitary(original: QuantumCircuit,
        ...
        try:
            ...
            if num_qubits_mapped != num_qubits_orig:
                return False, f"Cannot compare unitaries: different qubit counts ({num_qubits_orig} vs {num_qubits_mapped})"
            ...
            if equivalent:
                ...
                )**2 / (2**original_no_meas.num_qubits)**2
    ...
    def _extract_initial_layout(mapped: QuantumCircuit) -> dict[int, int] | None:
        ...
        if hasattr(mapped, '_layout') and mapped._layout is not None:
            ...
            if layout is not None:
                ...
                return {qubit.index: physical_idx 
    ...
    def validate_measurement(original: QuantumCircuit,
        ...
        probs_mapped = np.array([counts_mapped.get(outcome, 0) for outcome in all_outcomes])
        ...
        if tvd < 0.05:  # 5% threshold
            ...
            if mask.any():
                try:
                    chi2, p_value = chisquare(observed_counts[mask], expected_counts[mask])
                    if p_value >= p_threshold:
                        return True, f"Statistics match (TVD={tvd:.4f}, p={p_value:.4f})"
                except:
                    pass
